...
pub fn generate_server_interface(iface: &db::Interface, hooks: &db::Hooks) -> Result<String> {
    ...
    // Emit a hook line only when it carries a command, the same way the H/I
    // knobs above are omitted when unset. An empty `PostUp = ` is noise, and
    // omitting it lets the privileged helper reject *any* hook directive it
    // sees without having to distinguish an empty one from a real command —
    // see `privhelper::reject_hook_directives`.
    let mut hook_lines: Vec<String> = Vec::new();
    for (key, raw) in [
        ("PreUp", &hooks.pre_up),
        ...
        let v = expand(raw, &iface.device, &port, &iface.ipv4_cidr, &iface.ipv6_cidr);
        if !v.trim().is_empty() {
            hook_lines.push(format!("{key} = {v}"));
        }
    }
    let hooks_block = if hook_lines.is_empty() {
        String::new()
    } else {
        format!("\n{}", hook_lines.join("\n"))
    };
...
mod tests {
    ...
    /// Hooks as a real deployment has them: the shipped defaults set the
    /// nftables ruleset up, so `post_up`/`post_down` are never empty in
    /// practice. The tests below use the hook lines as ordering landmarks,
    /// which only means anything with a representative fixture — empty hooks
    ...
    /// Hooks with every slot empty, for the omission test.
    fn empty_hooks_fixture() -> db::Hooks {
        db::Hooks {
            pre_up: String::new(),
            post_up: String::new(),
            pre_down: String::new(),
    ...
    #[test]
    fn server_interface_omits_hook_lines_that_carry_no_command() {
        // An empty `PostUp = ` is noise, and omitting it is what lets the
        // privileged helper reject *any* hook directive it sees without
        // having to tell an empty one from a real command.
        let cfg = generate_server_interface(&iface_fixture(), &empty_hooks_fixture()).unwrap();
        for key in ["PreUp", "PostUp", "PreDown", "PostDown"] {
            assert!(!cfg.contains(key), "{key} should be omitted when empty:\n{cfg}");
        }
        // The header's own blank line is expected; what must not appear is a
        // dangling one where the omitted hooks used to be.
        assert!(!cfg.ends_with('\n'), "no trailing blank where hooks were:\n{cfg}");
    }
    ...
    fn awg3_server_config_renders_every_set_knob() {
        ...
        assert!(hpk < conf.find("PostUp").unwrap());
    }
