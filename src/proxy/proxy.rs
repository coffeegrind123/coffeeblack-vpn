...
use crate::proxy::session::SessionTable;
...
fn build_proxy_status_snapshot(
    ...
    let session_now_ms = crate::proxy::session::now_millis();
...
impl Proxy {
    ...
    async fn handle_probe(
        ...
        metrics_ref: &Option<Arc<crate::proxy::metrics::ClientMetrics>>,
    ) {
    ...
    fn try_spawn_dns_forward(
        ...
        query: &[u8],
        client_addr: SocketAddr,
        metrics: &Arc<crate::proxy::metrics::ClientMetrics>,
    ) -> bool {
    ...
    fn spawn_session_relay(&self, client_addr: SocketAddr, backend_sock: Arc<UdpSocket>) {
        ...
        let relay_buf_size = self.config.relay_buffer_size.clamp(
            crate::proxy::config::RELAY_BUFFER_SIZE_MIN,
            crate::proxy::config::RELAY_BUFFER_SIZE_MAX,
        );
