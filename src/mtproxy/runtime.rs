//! Extract the bundled telemt ELF onto disk so the supervisor has
//! something to `exec`. The blob is gzipped and embedded via
//! `include_bytes!`; we decompress on demand, verify against the SHA
//! recorded in `vendor/TELEMT_VERSION`, and `chmod 755` so the kernel
//! will execve it.
//!
//! Re-extraction is skipped when the on-disk ELF already matches the
//! embedded SHA — keeps the second-and-subsequent startups fast.
//!
//! Mirror of `src/xray/runtime.rs`. Kept structurally identical so a
...
use crate::mtproxy::{TELEMT_SHA256, TELEMT_VERSION};

/// Gzipped telemt ELF, picked at build time by `build.rs` based on
/// `CARGO_CFG_TARGET_ARCH`. ~5.6 MiB compressed; ~14 MiB extracted.
const TELEMT_GZ: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/telemt.gz"));

/// Idempotently materialise the bundled telemt ELF at
/// `<mtproxy_dir>/telemt`. Returns the absolute path. Safe to call
...
pub fn extract_bundled_binary() -> Result<PathBuf> {
    ...
    if actual != TELEMT_SHA256 {
        let _ = fs::remove_file(&tmp);
        return Err(anyhow!(
            "extracted telemt SHA-256 mismatch: expected {TELEMT_SHA256}, got {actual}. \
             vendor/TELEMT_VERSION is out of sync with vendor/telemt-linux-amd64.gz."
        ));
    }
    ...
    crate::info!(
        ...
        "Extracted bundled telemt binary"
    );
    Ok(target)
}
...
fn matches_embedded_sha(path: &Path) -> Result<bool> {
    ...
    Ok(sha256_file(path)? == TELEMT_SHA256)
}
